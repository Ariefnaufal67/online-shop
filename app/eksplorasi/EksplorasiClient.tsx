...
export default function EksplorasiClient() {
  ...
  const handleCardClick = (p: P) => {
    const slug = SLUG_MAP[p.name];
    if (slug) router.push(`/produk/${slug}`);
  };

  const handleAddCart = (e: React.MouseEvent, p: P) => {
    e.stopPropagation();
  ...
  const toggleWish = (e: React.MouseEvent, name: string) => {
    e.stopPropagation();
      ...
      <div className="flex">
        ...
        <div className="flex-1 px-6 py-8">
          ...
          <p className="text-xs mb-5" style={{ color: "var(--muted)" }}>
            {sorted.length} produk ditemukan — klik produk untuk lihat detail
          </p>

          {/* Grid */}
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
            {sorted.map((p, i) => {
              const hasDetail = !!SLUG_MAP[p.name];
              return (
                <div key={p.name}
                  className="rounded-2xl overflow-hidden border transition-all hover:-translate-y-1 group"
                  style={{ background: "var(--card-bg)", borderColor: "var(--border)", cursor: hasDetail ? "pointer" : "default" }}
                  ...
                  <div className="relative h-40 flex items-center justify-center" style={{ background: BG_LIST[i % 4] }}>
                    ...
                    {hasDetail && (
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                        ...
                        <span className="text-xs font-medium text-white rounded-xl px-3 py-1.5"
                          style={{ background: "rgba(14,12,10,0.65)" }}>Lihat Detail →</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="px-3 py-3">
                    ...
                    <div className="flex items-center justify-between gap-1">
                      ...
                      <div className="flex gap-1 items-center">
                        <button onClick={e => toggleWish(e, p.name)}
                          className="rounded-lg px-1.5 py-0.5 text-sm border"
                        ...
                        <button onClick={e => handleAddCart(e, p)}
                          className="rounded-xl px-2 py-1 text-[10px] font-medium text-white"
