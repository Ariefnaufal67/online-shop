      ...
      <nav className="flex items-center justify-between px-6 md:px-10 py-4 border-b sticky top-0 z-40"
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setSearchOpen(true)}
            ...
            <kbd className="text-[10px] px-1.5 py-0.5 rounded border ml-6"
          ...
          <button onClick={() => setSearchOpen(true)}
            ...
            style={{ background: "var(--cream)", border: "none", cursor: "pointer", fontSize: 16 }}>
            🔍
          </button>

          {/* Cart button */}
          <button onClick={handleCartClick}
            className="flex items-center gap-2 rounded-2xl px-4 py-2 text-sm font-medium transition-all relative"
            style={{
              background: cartFlash ? "var(--accent2)" : "var(--ink)",
              color: "var(--paper)", border: "none", cursor: "pointer", fontFamily: "inherit",
            }}>
